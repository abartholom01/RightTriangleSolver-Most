...
//Tolerence
//Tolerence lets us run through the code if and when we come across slight differences by a fractional or small disputable number between crucial numbers.
...
//sorting routine
//variables to swap the angles and sides with
...
if(isTriangle){
    if(isThreeSides){
        if(calcSideA * calcSideA + calcSideB * calcSideB == calcSideC * calcSideC){
            print("Your measurements(3 sides) create a right triangle.")
            isRightTriangle = true
        }else{
            print("Your measurements do not create a right triangle, please try again.")
        }
    } else if(isTwoSides) {
        if calcAngleC == 90{
            if (calcSideC > calcSideA && calcSideC > calcSideB) || calcSideC == 0{
                isRightTriangle = true
                print("Your measurement creates that there right triangular shape.")
            }else{
                print("Your measurements do not create a right triangle, please try again.")
            }
            ...
            if (calcSideC == 0){
                if (tolerence > abs(tan(calcAngleA * pi / 180) - (calcSideA / calcSideB))){
                    isRightTriangle = true
                    print("Your measurement creates that there right traingular shape.")
                }else{
                    isRightTriangle = false
                    print("Your measurements do not create that there right traingular shape, please try again.")
                }
            }else if(calcSideB == 0){
        ...
        if (calcAngleA + calcAngleB == 90 || calcAngleC == 90){
            isRightTriangle = true
            print("Your measurements create yerself a right traingular shape!")
        }else{
            isRightTriangle = false
            print("Your measurements do not create a right triangle, please try again.")
        }
